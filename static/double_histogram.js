...
function double_hist(path,service,occ,checked,start,end){
  ...
  function myfunction() {
    ...
    //d3.json("get_bins2?string="+encodeURIComponent(path)+"&number="+occ, function(error, bins2) {
      d3.json("get_bins/"+service+"/"+start+"/"+end, function(error, bins1) {
        ...
        d3.json("get_binsSub?string="+encodeURIComponent(path)+"&number="+occ+"&number1="+start+"&number2="+end, function(error, bins2) {
        if (error) throw error;
        console.log(bins1)
        console.log(bins2)
  ...
  barGroup.append("g")
    ...
    .on('end', function(d){
        ...
        var checked = compareElement.value
